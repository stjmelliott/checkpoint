...
window.clearTrail = function clearTrail(){ if(currentTrailLayer){map.removeLayer(currentTrailLayer); currentTrailLayer=null;}} window.filterByStatus = function filterByStatus(status,button){document.querySelectorAll('.status-tab').forEach(b=>b.classList.remove('active')); button.classList.add('active'); loadDashboardData(status);} 
