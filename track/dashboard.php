    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        ...
        <div style="display:flex; gap:8px;">
            <?php if (in_array($load_entry_mode, ['manual', 'both'])): ?>
                <button class="btn btn-success" id="ADD_LOAD_BTN" data-bs-toggle="modal" data-bs-target="#addLoadModal" style="background:#22c55e; border:none; padding:6px 12px; border-radius:4px; font-weight:600; color:#fff;">+ Add Load</button>
            <?php endif; ?>
...
function clearTrail(){ if(currentTrailLayer){map.removeLayer(currentTrailLayer); currentTrailLayer=null;}} function filterByStatus(status,button){document.querySelectorAll('.status-tab').forEach(b=>b.classList.remove('active')); button.classList.add('active'); loadDashboardData(status);} 
